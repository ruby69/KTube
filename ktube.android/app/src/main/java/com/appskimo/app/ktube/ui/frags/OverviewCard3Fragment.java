...
@EFragment(R.layout.fragment_overview_card3)
public class OverviewCard3Fragment extends Fragment {
    @ViewById(R.id.title) TextView titleView;
    @ViewsById({R.id.image1, R.id.image2, R.id.image3}) List<ImageView> imageViews;
    @ViewsById({R.id.title1, R.id.title2, R.id.title3}) List<TextView> titleViews;
    @ViewsById({R.id.info1, R.id.info2, R.id.info3}) List<TextView> infoViews;
    ...
    @Click({R.id.card1, R.id.card2, R.id.card3})
    ...
    @Click(R.id.more)
    void onClickMore() {
        if (videoDomain.getVideoGroup().isVideo() && !videoDomain.getFetchCategory().isBest()) {
            VideoActivity_.intent(getActivity()).videoGroup(videoDomain.getVideoGroup()).start();
        } else {
